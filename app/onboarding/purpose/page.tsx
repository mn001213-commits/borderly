...
export default function PurposeOnboardingPage() {
  ...
  const options = useMemo(
    () =>
      [
        { key: "community" as const, label: "Community / Make Friends" },
        { key: "volunteer" as const, label: "Volunteering / Skills Sharing" },
        { key: "help" as const, label: "Help & Counseling" },
        { key: "ngo" as const, label: "NGO Programs" },
        { key: "jobs" as const, label: "Jobs & Education" },
      ] as const,
    []
  );
  ...
  return (
    <div className="mx-auto max-w-xl p-6">
      <div className="rounded-2xl border bg-white/80 p-6 shadow-sm">
        <h1 className="text-xl font-semibold">Select your purpose</h1>
        <p className="mt-2 text-sm text-gray-600">Your selections help us recommend relevant posts and people.</p>
          ...
          {options.map((o) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm">{selected[o.key] ? "Selected" : ""}</span>
              </div>
            </button>
          ))}
        </div>
