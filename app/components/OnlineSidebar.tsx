...
import { User, Users, Home, FileText, Search, CalendarHeart, MessageCircle, ShieldAlert, ShieldCheck, Globe, ChevronRight, Settings, Building2 } from "lucide-react";
...
export default function OnlineSidebar() {
  ...
  const getPageInfo = (p: string) => {
    const map: Record<string, { titleKey: string; descKey: string }> = {
      ...
      "/users": { titleKey: "pageInfo.partners.title", descKey: "pageInfo.partners.desc" },
    };
    let keys = map[p];
    if (!keys) {
